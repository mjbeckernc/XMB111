...
proc means data=casuser.trial_simulation noprint;
   class patientID;
   var response_observed adverse_event_occurred;
   output out=casuser.trial_summary(drop=_:) 
...
data casuser.trial_summary;
  set casuser.trial_summary;
  where patientID ne .;
run;

/* Visualize the response distribution */
proc sgplot data=casuser.trial_summary;
...
data casuser.high_resp_freq_adv_prob;
   set casuser.trial_summary;
   highResponder = (sum_resp_obs >= &highResponseThreshold);
   frequentAdverse = (sum_ae_occur >= &highAdverseThreshold);
run;

proc freq data=casuser.high_resp_freq_adv_prob;
   tables highResponder frequentAdverse / out=casuser.probability_summary;
run;

proc print data=casuser.probability_summary;
